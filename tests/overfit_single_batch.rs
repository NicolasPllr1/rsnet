...
fn test_sgd_momentum_can_overfit_single_batch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    const MAX_OPTIM_STEPS: usize = 5000; // Generous. Worse cases seem to need ~15 epochs to converge.
    ...
    for optim_step in 1..(MAX_OPTIM_STEPS + 1) {
        ...
        if avg_loss < OVERFITTED_LOSS_VALUE {
            println!("Reached loss<{OVERFITTED_LOSS_VALUE} in {optim_step} optimisation steps",);
            return Ok(());
        }
    ...
    Err(format!(
        "Loss (reached {avg_loss}) did not go below the overfitting test threshold ({OVERFITTED_LOSS_VALUE})"
    ))?
}
